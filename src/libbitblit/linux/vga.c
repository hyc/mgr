...
#ifdef TSENG4K
/* tseng et4000 in diamond speedstar */
  {
    "800x600x2",800,600,4,
    {
#include "800x600x2.h"
...
#endif
#ifdef S3
/* S3 chipset in a fahrenheit orchid */
  {
    "800x600x2",800,600,4,
    {
#include "800x600x2S3.h"
    }
  },
#endif
#ifdef TRIDENT
...
static void tseng4k_setpage( int page ) {
	page &= 0x0F;
	port_out(last_page = (page | (page << 4)), TSENG4K_SEG_SELECT);
}


/* Bank switching function - set 64K read bank number */
static void tseng4k_setrdpage( int page ) {
	last_page &= 0x0F;
	last_page |= ((page & 0x0F) << 4);
	port_out(last_page, TSENG4K_SEG_SELECT);
}

/* Bank switching function - set 64K write bank number */
static void tseng4k_setwrpage( int page ) {
	last_page &= 0xF0;
	last_page |= (page & 0x0F);
	port_out(last_page, TSENG4K_SEG_SELECT);
}
...
#ifdef __FreeBSD__
static int iopermfd = -1;

static int
ioperm( unsigned long int from, unsigned long int num, int turn_on) {
  if( turn_on && iopermfd < 0)
    iopermfd = open( "/dev/io", O_RDONLY);
  return iopermfd < 0? -1: 0;
}
#endif
...
bit_initscreen(char *name, int *width, int *height, unsigned char *depth, void **devi)
{
  ...
  /*}}}  */
	...
	return((DATA*)0);
      ...
      if ((graph_mem = malloc(GRAPH_SIZE*2 + pagesize)) == NULL) {
        printf("init: allocation error\n");
        exit(-1);
      }
      if ((unsigned long)graph_mem % pagesize)
        graph_mem += pagesize - ((unsigned long)graph_mem % pagesize);
